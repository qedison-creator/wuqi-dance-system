...
// 拼接完整图片URL：若为相对路径则加上服务器前缀
function fixImageUrl(url, serverBase) {
  if (!url) return '';
...
Page({
  data: {
    ...
    selectedIds: [],
    selectAll: false,
    // 权限控制：仅超管(super_admin)和审核员(reviewer)可操作（编辑/删除/上传）
    can_operate: false,
    // 公共画册固定 formStoreId 为空字符串（不传 store_id）
    formStoreId: '',
  },
  ...
  _initPermission() {
    ...
    const can_operate = role === 'super_admin' || role === 'reviewer';
    this.setData({ can_operate });
  },

  async loadData() {
    this.setData({ loading: true });
    try {
      // 公共画册：固定 gallery_type=public，不传 store_id
      const params = { pageSize: 100, gallery_type: 'public' };
      ...
      list.forEach(item => {
        ...
        // 拼接完整图片URL
        item.thumbnail_url = fixImageUrl(item.thumbnail_url, serverBase);
        item.image_url = fixImageUrl(item.image_url, serverBase);
        // 拼接教练名
